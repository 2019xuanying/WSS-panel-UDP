...
# V2.6.0 (Axiom - TG Bot Integration)
#
# [CHANGELOG]
# - [FEATURE] 集成 Telegram Bot 配置向导。
...
TG_BOT_LOGIC_DEST="$PANEL_DIR/tg_bot_logic.js" # [TG_BOT]
...
cd "$PANEL_DIR"
# [TG_BOT] 安装 node-telegram-bot-api
...
if [ -f "$REPO_ROOT/tg_bot_logic.js" ]; then
    ...
    # 兼容性：如果用户只更新了 install.sh 但没下载新文件，尝试生成一个空的逻辑文件防止报错
    if [ ! -f "$TG_BOT_LOGIC_DEST" ]; then
        echo "module.exports = { initTelegramBot: () => console.log('TG Bot logic missing') };" > "$TG_BOT_LOGIC_DEST"
    fi
fi
...
echo "=================================================="
echo "✅ 部署完成！(Axiom V2.6.0 - TG Bot Config)"
echo "   - UDP Custom: 0.0.0.0:$UDP_CUSTOM_PORT"
echo "   - BadVPN: 127.0.0.1:$UDPGW_PORT"
echo "   - TG Bot: ${TG_BOT_TOKEN:+ENABLED (Admin: $TG_ADMIN_ID)}"
echo "=================================================="
